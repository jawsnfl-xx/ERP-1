	...
	if ($this->_parameters[1] === 'view') {
		...
		if ($this->_parameters[2] === '1') {
			...
			if ($session->getup("product/add/value/number")) {
				...
				/**
				 * Nic nie wiemy o detalu, jaki dodajemy... Źle. Cofamy się do kroku 1.
				 */
		...
		if ($this->_parameters[2] === '1') {
			...
			if (! empty($_keywords)) {
				...
				if (! empty($this->_table['product']['add']['exists'])) {
					/**
					 * Istnieje jakiś produkt to takim identyfikatorze. Należy się cofnąć i zaproponować przejście do p/view oraz zmianę identyfikatora
					 */
					$session = Registry::get("session");
				...
				/**
				 * Wpisana wartość nie jest liczbą. Wracamy...
				 */
				$session = Registry::get("session");
		...
		/**
		 * Budowa pagera UWAGA! Napisać na szybko, później przenieść jako uniwersalny element.../ $count = $product->_createListCount();
		...
		if ($this->_parameters[2]) {
			...
			if ($product->_isExistsByID($this->_parameters[2])) {
				/**
				 * Sprawdzamy, czy detal o takim idzie istnieje. To ważne, bo co możemy edytować, jak nie będzie produktu w bazie danych.
				 */
				$this->_table['product']['edit'] = $product->_createView($this->_parameters[2]);
				// var_dump($this->_table['product']['edit'] );
			} else {
				/**
				 * Nie mamy takiego detalu. Można wyświetlić info o błędzie, albo wrócić do product/page
				 */
				header("Location: /module/product_technology/product/page/");
				die();
			}
			...
			/**
			 * Nie wprowadzono parametru. Hmmm. Strona wyświetlona niepotrzebnie. Wracamy do product/page
			 */
			header("Location: /module/product_technology/product/page/");
			die();
		}
		...
		/**
		 * W wypadku wyświetlenia dowolnej ze stron pt/product wyświerla listę stron z defaultową pierwszą stroną. Zawiera ona 20 wyników.
		 */
		...
		public function quality_management() {
			...
			if ($this->_parameters[0] === 'review') {
				...
				/**
				 * Tymczasowo ładowany widok ControlSheet. Do rozbudowy.
				 */
			...
			elseif ($this->_parameters[0] === 'add') {
				...
				/**
				 * Wyświetla arkusz kontroli pomiaru. Na tym etapie musi być podzielony na 3 etapy. 1. wprowadzenie danych początkowych 2. wprowadzenie pomiarów wymiarów kontolnych 3. wprowadzenie pomiarów tolerancji geometrii i kształtu
				 * Do tego dochodzi jeszcze podsumowanie, ale to nie na teraz... :)
				 * Lol - jak zabawnie :D/
				
				if ($this->_parameters[1] === 'step1') {
					/**
					 * Krok 1. Widok robi pola do wpisania informacji Kontroler chyba nie będzie robił nic... Nie nie i nie... ! Kontroler musi sprawdzić, czy nie zostały do niego przekazane błedy dotyczące brakujących lub niepoprawnych danych z niego samego z _step1
					...
					if (RequestMethods::get("form_err")) {
						/**
						 * No to mamy jakieś błędy. Trzeba sprawdzić poprawność tego co tam jest i zbudować tablicę.
						 */
					...
					/**
					 * Sprawdzenie Krok 1. Musi sprawdzić zawartość wszystkich pól w które zostały wpisane informacje. Jeśli są te, które niezbędne są do założenia karty pomiaru, robi to. Jeśli brakuje wymaganych informacji wróci do poprzedniej strony i zakomunikuje które pola były brzydkie, a które puste.
					 */
					...
					if ($tmp_error) {
						...
						/**
						 * Cholerka... Wszystko zapowiada się dobrze. Poza otworzyć arkusz (sheet) kontroli jakości... :)
						 */
						
						/**
						 * Po utworzeniu arkusza należy przejść do kroku 2... Trzeba zastanowić się tylko jak przekazać identyfikator nowego arkusza :D
						 */
						header('Location: /module/quality_management/add/step2');
					}
				} elseif ($this->_parameters[1] === 'step2') {
				/**
				 * Krok 2. Widok wyświetla tabelę do pomiarów zgodną z wpisanym wcześniej detalem. Kontroler przelicza rozmiar tabeli, czyta pola wymiarów, tolerancji itp., utrzymuje rozpoczęta kartę pomiaru.
				 */
				...
				/**
				 * Sprawdzenie podsumowania. Wydaje mi się, że na sprawdzeniu podsumowania powinien przelecieć jeszcze raz spójność podsumowania oraz przejść do review z podkreślonym danym wspisem, albo do view z danym wpisem.
				 */
				} else {}
			} else {}
		}

...
namespace Plugins\Product_technology {
	...
	class Product extends Product_technology {
		...
		public function _createSoftList($_pageNumber = 1, $_limitAtPage = 20) {
			...
			$queryBuilder->select ( '*' )->from ( 'products', 'products' )->leftJoin ( "products", "units", "units", "units.id_units = products.units_id_units" )->leftJoin ( "products", "category_product", "category_product", "category_product.product_id_product = products.id_products" )->leftJoin ( "products", "category", "category", "category.id_category = category_product.category_id_category" )->orderBy ( 'products.products_name', 'ASC' )->setFirstResult ( $limit_down )->setMaxResults ( $limit_up );
		...
		public function _createView($_id) {
			...
			$queryBuilder->select ( '*' )->from ( 'products', 'products' )->leftJoin ( "products", "units", "units", "units.id_units = products.units_id_units" )->leftJoin ( "products", "category_product", "category_product", "category_product.product_id_product = products.id_products" )->leftJoin ( "products", "category", "category", "category.id_category = category_product.category_id_category" )->where ( $queryBuilder->expr ()->eq ( "products.id_products", $_id ) )->setMaxResults ( 1 );
		...
		public function _isExists($_name) {
			...
			$queryBuilder->select ( '*' )->from ( 'products', 'products' )->where ( $queryBuilder->expr ()->eq ( "products.products_name", $_name ) )->setMaxResults ( 1 );
		...
		public function _isExistsByID($_id) {
			...
			$queryBuilder->select ( '*' )->from ( 'products', 'products' )->where ( $queryBuilder->expr ()->eq ( "products.id_products", $_id ) )->setMaxResults ( 1 );
		...
		public function _createListUnits() {
			...
			$queryBuilder->select ( '*' )->from ( 'units', 'units' )->setMaxResults ( 100 );
		...
		public function _createListCategory() {
			...
			$queryBuilder->select ( '*' )->from ( 'category', 'category' )->setMaxResults ( 100 );
